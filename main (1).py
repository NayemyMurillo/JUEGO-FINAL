...
fondo_menu= pygame.image.load("fondo_menu.png")
...
#VARIABLES DEL MENU
fuente_opciones = pygame.font.Font("Inconsolata.ttf", 50)  
...
class Bala(pygame.sprite.Sprite): #herencia de pygame
    ...
    def update(self,personaje,misiles):
        ...
        for misil in misiles:
            if self.rect.colliderect(misil.rect):
                misiles.remove(misil)  # Eliminar el misil de la lista
                self.kill()
...
def dibujar_menu():
    ...
    for i, opcion in enumerate(opciones):
        texto = fuente_opciones.render(opcion, True, (0, 0, 0))  # Color negro
        x_opcion = ancho // 2 - texto.get_width() // 2
        y_opcion = alto // 2 + i * (fuente_opciones.get_height() + 20)

        # Cuadro alrededor de la opción
        cuadro_rect = pygame.Rect(x_opcion - 10, y_opcion - 5, texto.get_width() + 20, texto.get_height() + 10)
        if i == opcion_seleccionada:
            pygame.draw.rect(screen, (0, 128, 250), cuadro_rect, border_radius=5)  # Plomizo cuando está seleccionado
        else:
            pygame.draw.rect(screen, (255, 255, 255), cuadro_rect, border_radius=5)  # Cuadro negro cuando no está seleccionado
...
while menu_abierto:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            for i, opcion in enumerate(opciones):
                texto = fuente_opciones.render(opcion, True, (51, 155, 255))
                x_opcion = ancho // 2 - texto.get_width() // 2
                y_opcion = alto // 2 + i * (fuente_opciones.get_height() + 20)
                opcion_rect = pygame.Rect(x_opcion - 10, y_opcion - 5, texto.get_width() + 20, texto.get_height() + 10)
...
while loop:
    ...
    if keys[pygame.K_a] and jugador.forma.x > 0:  # Evita salir del borde izquierdo
        delta_x = -velocidad_jugador
    if keys[pygame.K_d] and jugador.forma.x < ancho - jugador.forma.width:  # Evita salir del borde derecho
        delta_x = velocidad_jugador
    if keys[pygame.K_s] and jugador.forma.y < alto - jugador.forma.height:  # Evita salir del borde inferior
        delta_y = velocidad_jugador
    if keys[pygame.K_w] and jugador.forma.y > 0:  # Evita salir del borde superior
        delta_y = -velocidad_jugador
